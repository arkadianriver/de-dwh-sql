...
case $1 in
    ...
    "load")     sudo -u postgres psql -f bronze/load_bronze.sql 2>&1 | tee "${logroot}-${tstamp}-3-load.log"
                ;;
    "all")      sudo -u postgres psql -f init_database.sql  2>&1 | tee "${logroot}-${tstamp}-1-init.log" &&
                sudo -u postgres psql -f bronze/ddl_bronze.sql 2>&1 | tee "${logroot}-${tstamp}-2-create.log" &&
                sudo -u postgres psql -f bronze/load_bronze.sql 2>&1 | tee "${logroot}-${tstamp}-3-load.log"
                ;;
