...
we'll be creating views into the cleaned silver tables, views to integrate and possibly aggregate information.
...
the links between the relevant columns in the gold views
(rather than defining those columns as foreign keys).
Only after JOINing the fact table are the foreign key relationships revealed.

To verify the joins (foreign key integrity) execute joins on the keys and to ensure none of them are NULL:
...
After coding the gold layer, we end up with this data model for our star schema:
