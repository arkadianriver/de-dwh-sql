...
We'll add a `created` column to each silver table. Following the data engineer naming conventions,
...
[source,sql]
----
include::../silver/qry_check_for_dupe_keys.sql[]
----
...
=== `crm_prd_info` and `crm_sales_details` tables
...
=== `erp_cust_az12`,`erp_loc_a101`, and `erp_px_cat_g1v2` tables
